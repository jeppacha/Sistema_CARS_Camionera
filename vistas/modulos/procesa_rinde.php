	...
	$directorio_base = "/var/www/html/cars/vistas/archivos/bajarepar/M";
	...
	while($archivo = readdir($dir_handle)) {
  	...
  	$stmt = $con->prepare("DELETE FROM rindehrepar");
	...
	$sqlhrep = $con->prepare("SELECT * FROM hrepartos WHERE codoff = '8' AND cantg = guiarend AND enviado is null" );
	...
	foreach ($resultahrep as $key => $value) {
		...
		//var_dump($value);
		$stmt = $con->prepare("INSERT INTO rindehrepar (codoff, cuitfle, repnro, completo) VALUES (:codoff,:cuitfle,:repnro,:completo)");
	...
	$sqlrinde = $con->prepare("SELECT * FROM rindehrepar" );
	...
	foreach($rindehrep as $value){
		...
		$file = "compM".$value["repnro"].".txt"; //le doy un nombre al archivo
		$filedest = "compM".$value["repnro"].".txt";
	...
	if(filesize($file) > 0){
 	...
 	if (file_exists($directorio_arch."/".$file)) {
