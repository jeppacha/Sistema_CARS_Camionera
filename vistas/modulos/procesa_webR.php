<?php 
	...
	$directorio_base = "/var/www/html/cars/vistas/archivos/suberepar/R" ;
...
//Archivo DETALLE REPARTO
				  	...
				  	while ($datadrep = fgetcsv($detallerep, 1000, ",")){
				    	...
				    	if(isset($datadrep[0])){
				    		...
				    		$sqldrep1 = $con->prepare("SELECT * FROM repartos WHERE codoff = '$datadrep[1]' AND nguia = '$datadrep[5]'");
				    		
				    		$sqldrep1 ->execute();

				    		$resultarep1 = $sqldrep1 -> fetch();

				    		if($resultarep1 != ""){

				    			if($resultarep1[7] == null){
					    			
					    			$stmt = $con->prepare("DELETE FROM repartos WHERE nguia = '$resultarep1[5]'");

				    				if($stmt->execute()){

									echo "<br />".'Registro DETALLE REPARTOS guia '.$resultarep1[5].' Borrado';

							    	}else{

									echo "<br />".'Registro DETALLE REPARTOS guia '.$resultarep1[5].' NO Borrado';
							    	}

						    	}

						    	if(($resultarep1[7] == 0) && ($resultarep1[3] != $datadrep[3])){

						    		$stmt = $con->prepare("DELETE FROM repartos WHERE codoff = '$resultarep1[1]' AND nguia = '$resultarep1[5]'");
				    		...
				    		$resultarep = $sqldrep -> fetch();

				    		if($resultarep != ""){
				    		
				    			if($resultarep[7] == null){
				    ...
				    if(file_exists($directorio."/fleteros".$subnamerep.".asc")){
					  	...
					  	while ($dataflet = fgetcsv($fletero, 1000, ",")){
					    	...
					    	if(isset($dataflet[0])){
					    		...
					    		$resultafle = $sqlflet -> fetch();

					    		if($resultafle != ""){

					    			$paswd = $resultafle[5];
					    			$fotofle = $resultafle[8];
					    			$alta = $resultafle[9];
					    			$ultimo = $resultafle[10];
									...
									$stmt->bindParam(':foto',$fotofle, PDO::PARAM_STR);
									$stmt->bindParam(':fecha_alta',$alta, PDO::PARAM_STR);
									$stmt->bindParam(':ultimo_login',$ultimo, PDO::PARAM_STR);
				    ...
				    if(file_exists($directorio."/clientes".$subnamerep.".asc")){
					  	...
					  	while ($datacli = fgetcsv($cliente, 1000, ",")){
					    	...
					    	if(isset($datacli[0])){
					    		...
					    		$resultacli = $sqlcli -> fetch();

					    		if($resultacli != ""){
					    		
					    			$stmt = $con->prepare("UPDATE clientes SET nombre = '$datacli[3]', email1 = '$datacli[5]', email2 = '$datacli[6]', email3 = '$datacli[7]', cuit = '$datacli[8]' WHERE codcli = '$datacli[1]'");
					    			...
					    			if($stmt->execute()){
										...
										echo "<br />".'Registro Cliente '.$datacli[1].' Modificado';

								    }else{

										echo "<br />".'Registro Cliente '.$datacli[1].' NO Modificado';
								    }
									...
									//$stmt->bindParam(':password',$resultacli[4], PDO::PARAM_STR);
				    ...
				    if(file_exists($directorio."/subecars".$subnamerep.".asc")){
					  	...
					  	while ($datacar = fgetcsv($cars, 1000, ",")){
					    	...
					    	if(isset($datacar[0])){
					    		...
					    		//var_dump($datahrep);
					    		//echo $datacar[4];
					    		$sqlcar = $con->prepare("SELECT * FROM tb_cars WHERE codcli = '$datacar[1]' AND nguia = '$datacar[4]'");
					    		
					    		$sqlcar ->execute();

					    		$resultacar = $sqlcar -> fetch();
					    		...
					    		if($resultacar != ""){
					    			...
					    			if($resultacar[13] == 1){
										...
										echo "<br />".$resultacar[4];
										$fecha_actual = date('Y-m-d');
										$fechalimit = date('Y-m-d',strtotime($fecha_actual."- 2 month"));
										if($resultacar[12]<$fechalimit){ 
										
											//echo $fechalimit;
											$stmt = $con->prepare("DELETE FROM tb_cars WHERE nguia = '$resultacar[4]' ");
					    			...
					    			if($resultacar[13] == null){

					    				if($resultacar[11] != $datacar[11]){
					    		
						    				$stmt = $con->prepare("UPDATE tb_cars SET nomrem = '$datacar[7]', nomdes = '$datacar[8]', reparto = '$datacar[11]' WHERE codcli = '$resultacar[1]' AND nguia = '$resultacar[4]'");

						    				if($stmt->execute()){

												echo "<br />".'Registro CARS '.$resultacar[4].' Modificado';

									    	}else{

												echo "<br />".'Registro CARS '.$resultacar[4].' NO Modificado';
									    	}

									    }else{

						    				$stmt = $con->prepare("UPDATE tb_cars SET nomrem = '$datacar[7]', nomdes = '$datacar[8]' WHERE codcli = '$resultacar[1]' AND nguia = '$resultacar[4]'");
					    			...
					    			if($datacar[1] != null){
										...
										if($stmt->execute()){
						    				...
						    				echo "<br />".'Registro CARS AGREGADO CON EXITO 3';
											
									    }else{

											echo "<br />".'Registro CARS NO AGREGADO 3';
